...
2025/03/27
작성자: colabbear
1.  칼럼별로 dtype 변경을 하고 NaN 처리된 빈칸에 대해 결측치를 넣는 것을
    반대로 NaN 처리된 빈칸에 9999. 결측값을 넣고 형변환을 진행하도록 변경
2.  기상청자료개방포털에서 파일셋으로 다운 받는 경우 QC플래그 칼럼이 없으므로 각 함수마다 QC플래그 처리 부분 삭제함
3.  전운량 결측인 경우 9999가 아닌 0으로 넣는 것으로 변경
    (특정 시간에 대해 한 기상관측값이 모든 지점에서 결측치가 되는 경우 모델이 돌아가지 않는 것을 방지)
    (0으로 처리하는 논리가 부족하긴 함)
...
# 기상자료개방포털에서 파일셋 다운로드로 획득한 asos 시간 자료 파일인 경우를 가정함
    ...
    for root, dirs, files in os.walk(source_folder):
        for file in files:
            if file.endswith('.csv'):
                ...
                # 기상청자료개방포털에서 파일셋으로 다운 받는 경우 QC플래그 칼럼이 없으므로 삭제
                ...
                df.loc[df["풍향(16방위)"].isna(), "풍향(16방위)"] = 9999.
                ...
                # 모든 지점이 결측인 경우를 예방하기 위해 빈칸을 0으로 처리 (0으로 처리하는 논리가 부족하긴 함)
                df.loc[df["전운량(10분위)"].isna(), "전운량(10분위)"] = 0.
...
# 기상자료개방포털에서 파일셋 다운로드로 획득한 aws 시간 자료 파일인 경우를 가정함
...
# 기상자료개방포털에서 파일셋 다운로드로 획득한 공공기관 기상관측 분 자료 파일인 경우를 가정함
def read_AWOS(source_folder):
    ...
    for stnID in df_temp:
        ...
        # 칼럼별로 dtype 적절하게 변경
        df_temp[stnID] = df_temp[stnID].astype({"상대습도(%)": "int64"})
